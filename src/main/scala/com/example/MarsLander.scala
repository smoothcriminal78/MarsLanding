...
class MarsLander {
  ...
  def plotRoute(q: Queue[LandingData]): Queue[LandingData] = {
    ...
    if (y > alt) {
      ...
      if (x < p1 || x > p2) {
        println(s"ca $ca fa $fa")
        val ga =
        //          if (ca > 0 && ca < 37) 36
        //        else if (ca < 0 && ca > -37) -36
          if (ca > 0 && ca < 55) 36
          else if (ca < 0 && ca > -55) -36
          //        else if (ca >0 && ca < 40) 39
          //        else if (ca < 0 && ca > -40) -39
          else if (ca > 0 && ca >= 55) 22
          else if (ca < 0 && ca <= -55) -22
          else ca

        val ab = braking(Queue(e.copy(t = 2)), -ga)
        if (ab._1.isEmpty) {
          q += nextPoint(e)
        } else {
          q ++= ab._1
  ...
  def levelOff(q: Queue[LandingData]): Queue[LandingData] = {
    ...
    val na = if (e.hs > 0) 15 else if (e.hs < 0) -15 else 0
    e.ang = rangeAng(e.ang, na)
  ...
  def braking(q: Queue[LandingData], a: Int): (Queue[LandingData], Int) = {
    ...
    if (hs == 0 || e.x == np.x || e.x == np.x + np.hs || y <= alt || x < 0) {
      ...
      if (y >= alt && x >= p1 && x <= p2) (q.tail, a) else (Queue(), -1)
    }
    else braking(q, a)
  }
  ...
  def calcThrottle(data: LandingData) = {
    ...
    if (vs < -39) 4
    else if (t == 2 && ((hs < 0 && ang < 0) || (hs > 0 && ang > 0))) 4
    //    else if (t == 1 && vs> -39) 2
    else if (ang == 0) 4
    else if (ang <= -22 || ang >= 22) 4
    else 3
  }
}
